...
/*   Updated: 2024/03/15 14:23:38 by lofiorin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_select_image(t_data *window, char c, int x, int y)
{
	...
	else if (c == 'P')
	{
		...
		put_img(window, window->img.enemy[0], x, y);
}
...
void	ft_render(t_data *window)
{
	...
	window->img.enemy[0] = xpm_to_img(window, "img/enemy_f.xpm", i, j);
	window->img.enemy[1] = xpm_to_img(window, "img/enemy_b.xpm", i, j);
	window->img.enemy[2] = xpm_to_img(window, "img/enemy_l.xpm", i, j);
	window->img.enemy[3] = xpm_to_img(window, "img/enemy_r.xpm", i, j);
